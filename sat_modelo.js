...
function doSolve(clauses, assignment) {
  ...
  while ((!isSat) && iter<=Math.pow(2,n)) {
    ...
    for(let k=0;k<clauses.length&&clauseTrue==1;k++){//for each clause
      lauseTrue = false//will be true if one of the clause's variables is true
...
function nextAssignment(currentAssignment) {// Receives the current assignment and changes it to the next one
...
function readVariables(clauses){//return variables = [0,0,0,0,0.....0,0,0] with length equals to the maximum absolute value between all variables in the clauses
  ...
  variables = []
  for(let i =1; i<=max;i++)//creating first assignment
    variables.push(0)
